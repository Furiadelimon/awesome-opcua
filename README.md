...
- [otlab-sim](https://github.com/Furiadelimon/otlab-sim) - open source (MIT) YAML-configured simulator exposing the same simulated process as an OPC UA server and a Modbus TCP server, with realistic value behaviours and timed fault injection (stuck/spike/drift/offline). Runs in Docker.
